...
// ==========================================
// 3. GESTION DES COMPTES ET NAVIGATION
// ==========================================
...
// ==========================================
// 4. FONCTIONS CLIENT (VRAIES FONCTIONS)
// ==========================================
        ...
        snapshot.forEach((doc) => {
            ...
            html += `<div class="saved-item" style="flex-direction: column; align-items: flex-start;">
                ...
                <ul style="color: #ccc; font-size: 0.85rem; margin-bottom: 10px; width: 100%;">
                    ${cmd.articles.map(art => `<li>• ${art.quantite}x ${art.nom} (Taille ${art.taille}) - ${art.prix * art.quantite} €</li>`).join('')}
                </ul>
                <strong>Total : ${cmd.total} € ${cmd.codePromo !== 'Aucun' ? `(Promo: ${cmd.codePromo})` : ''}</strong>
            </div>`;
        });
        vue.innerHTML = html;
    });
...
window.demanderRetour = async (idCommande, email) => { 
    await db.collection('retours').add({ email: email, idCommande: idCommande, date: firebase.firestore.FieldValue.serverTimestamp(), statut: 'En attente' }); 
    afficherNotification("Demande envoyée !"); 
}
...
window.sauvegarderAdresse = async (e) => { 
    e.preventDefault(); 
    const adresse = `${document.getElementById('addr-rue').value}, ${document.getElementById('addr-cp').value} ${document.getElementById('addr-ville').value}`;
    await db.collection('adresses').add({ email: auth.currentUser.email, adresse: adresse }); 
    document.getElementById('form-address').reset(); document.getElementById('form-address').style.display='none'; document.getElementById('btn-show-address').style.display='block';
    chargerAdresses(); afficherNotification("Adresse ajoutée"); 
}
window.chargerAdresses = () => { 
    db.collection('adresses').where('email','==',auth.currentUser.email).get().then(snap => { 
        let h=''; snap.forEach(doc => h+=`<div class="saved-item">📍 <span>${doc.data().adresse}</span></div>`); 
        document.getElementById('adresses-list').innerHTML = h||'<p class="empty-state">Aucune adresse.</p>'; 
    }); 
}
...
window.sauvegarderCarte = async (e) => { 
    e.preventDefault(); 
    await db.collection('cartes').add({ email: auth.currentUser.email, chiffres: document.getElementById('card-numero').value.slice(-4) }); 
    document.getElementById('form-card').reset(); document.getElementById('form-card').style.display='none'; document.getElementById('btn-show-card').style.display='block';
    chargerCartes(); afficherNotification("Carte ajoutée"); 
}
window.chargerCartes = () => { 
    db.collection('cartes').where('email','==',auth.currentUser.email).get().then(snap => { 
        let h=''; snap.forEach(doc => h+=`<div class="saved-item">💳 <span>Carte terminant par **** ${doc.data().chiffres}</span></div>`); 
        document.getElementById('cartes-list').innerHTML = h||'<p class="empty-state">Aucune carte.</p>'; 
    }); 
}
...
window.creerNouveauTicket = async (e) => { 
    ...
    await db.collection('support').add({ email: auth.currentUser.email, sujet: document.getElementById('sujet-support').value, statut: 'En attente', dateMAJ: Date.now(), messages: [{ sender: auth.currentUser.email, text: msg, date: Date.now() }]}); 
    document.getElementById('form-support').reset(); cacherFormulaireTicket(); afficherNotification("Ticket envoyé !"); chargerSupportClient(); 
}

window.chargerSupportClient = () => {
    if(!auth.currentUser) return;
    document.getElementById('support-conversation').style.display = 'none'; document.getElementById('support-tickets-list').style.display = 'block'; document.getElementById('btn-show-ticket-form').style.display = 'block';
    ...
    db.collection('support').where('email', '==', auth.currentUser.email).orderBy('dateMAJ', 'desc').get().then(snap => {
        ...
        snap.forEach(doc => {
            ...
            html += `<div class="saved-item" style="justify-content: space-between; cursor: pointer;" onclick="ouvrirConversation('${doc.id}')">
                        <div><strong>${t.sujet}</strong><br><span style="font-size: 0.8rem; color: #ccc;">Dernière modif le ${new Date(t.dateMAJ).toLocaleDateString()}</span></div>
                        <span style="color: ${color}; font-size: 0.8rem; font-weight: bold;">${t.statut}</span>
                    </div>`;
        });
...
window.ouvrirConversation = async (ticketId) => {
    ...
    document.getElementById('support-tickets-list').style.display = 'none'; document.getElementById('btn-show-ticket-form').style.display = 'none';
    document.getElementById('support-conversation').style.display = 'block';
    
    const doc = await db.collection('support').doc(ticketId).get(); const ticket = doc.data();
    document.getElementById('conv-sujet').innerText = ticket.sujet;
    
    let htmlMsg = '';
    ticket.messages.forEach(m => {
        let isFuture = m.sender === 'FUTURE';
        let align = isFuture ? 'flex-start' : 'flex-end'; let bg = isFuture ? '#fff' : '#333'; let txtColor = isFuture ? '#000' : '#fff'; let senderName = isFuture ? '👑 FUTURE' : 'Moi';
        htmlMsg += `<div style="align-self: ${align}; background: ${bg}; color: ${txtColor}; padding: 10px; border-radius: 8px; max-width: 80%;">
    ...
    if (ticket.statut === 'Clôturé') {
        ...
        // Ajout du bouton Clôturer client
        if (!document.getElementById('btn-close-ticket-client')) {
            const btnClose = document.createElement('button');
            btnClose.id = 'btn-close-ticket-client'; btnClose.className = 'btn-style'; btnClose.style.cssText = 'background: #f44336; color: #fff; padding: 10px; border: none; margin-top: 5px; cursor: pointer; border-radius: 5px;';
            btnClose.innerText = '🔒 CLÔTURER CE TICKET'; btnClose.onclick = () => fermerTicket(ticketId, 'client');
...
window.repondreAuTicket = async () => {
    ...
    await db.collection('support').doc(currentTicketId).update({ statut: 'En attente', dateMAJ: Date.now(), messages: firebase.firestore.FieldValue.arrayUnion({ sender: auth.currentUser.email, text: text, date: Date.now() }) });
    document.getElementById('reply-support-client').value = ''; ouvrirConversation(currentTicketId);
}

window.fermerTicket = async (ticketId, role) => {
    if(!confirm("Es-tu sûr de vouloir clôturer ce ticket ?")) return;
    await db.collection('support').doc(ticketId).update({ statut: 'Clôturé', dateMAJ: Date.now() });
    afficherNotification("Ticket clôturé !");
    if (role === 'client') ouvrirConversation(ticketId); else chargerAdminSupport(); 
}

// ==========================================
// 5. ESPACE PATRON (ADMIN)
// ==========================================
window.chargerAdminCommandes = async () => { 
    const div = document.getElementById('admin-content'); div.innerHTML = '<p class="empty-state">Chargement...</p>'; 
    const snap = await db.collection('commandes').orderBy('date', 'desc').get(); let html = ''; 
    snap.forEach(doc => { 
        const c = doc.data(); 
        const date = c.date ? c.date.toDate().toLocaleDateString('fr-FR') : 'Inconnue';
        html += `<div class="saved-item" style="flex-direction: column; align-items: flex-start; border-color: #555;">
                    <div style="width:100%; display:flex; justify-content:space-between; margin-bottom:5px;"><strong>${c.email}</strong><span style="color:#4CAF50;">${c.statut}</span></div>
                    <span style="color: #ccc; font-size: 0.8rem;">${c.total} € | Le ${date} | Promo: ${c.codePromo || 'Aucun'}</span>
                </div>`; 
    }); 
...
window.chargerAdminRetours = async () => { 
    const div = document.getElementById('admin-content'); div.innerHTML = '<p class="empty-state">Chargement...</p>'; 
    const snap = await db.collection('retours').orderBy('date', 'desc').get(); let html = ''; 
    snap.forEach(doc => { 
        const r = doc.data(); 
    ...
    div.innerHTML = html || '<p class="empty-state">Aucun retour.</p>'; 
}

window.chargerAdminSupport = async () => {
    const div = document.getElementById('admin-content');
    div.innerHTML = '<p class="empty-state">Chargement des tickets...</p>';
    const snap = await db.collection('support').orderBy('dateMAJ', 'desc').get();
    let html = '';
    snap.forEach(doc => {
        const t = doc.data();
        let color = t.statut === 'Répondu' ? '#4CAF50' : (t.statut === 'Clôturé' ? '#f44336' : '#FF9800');
        let actionsHtml = t.statut !== 'Clôturé' ? `
            <div style="display: flex; gap: 10px; margin-top: 10px;">
                <input type="text" id="reply-admin-${doc.id}" placeholder="Répondre..." style="flex: 1; padding: 8px; background: #000; color: #fff; border: 1px solid #444; border-radius: 4px;">
            ...
            </div>` : `<div style="color: #f44336; font-size: 0.8rem; margin-top: 10px; font-weight: bold; text-align: center;">🔒 TICKET CLÔTURÉ</div>`;

        html += `
        <div style="background: #1a1a1a; padding: 15px; border-radius: 8px; margin-bottom: 10px; border: 1px solid #555;">
            <div style="display: flex; justify-content: space-between; margin-bottom: 10px;"><strong>De: ${t.email}</strong><span style="color: ${color};">${t.statut}</span></div>
...
window.envoyerReponseAdmin = async (ticketId) => {
    ...
    if(!text) return;
    await db.collection('support').doc(ticketId).update({ statut: 'Répondu', dateMAJ: Date.now(), messages: firebase.firestore.FieldValue.arrayUnion({ sender: 'FUTURE', text: text, date: Date.now() }) });
    afficherNotification("Réponse envoyée !"); chargerAdminSupport(); 
}
...
// ==========================================
// 6. GESTION DU PANIER (AVEC TAILLES ET QUANTITÉS)
// ==========================================
let panier = [];
...
window.ajouterAuPanier = function(nom, prix, taille) {
    ...
    else panier.push({ nom: nom, prix: prix, taille: taille, quantite: 1 });
...
window.appliquerPromo = async () => {
    ...
    if(snap.empty) {
        msgBox.style.color = "#f44336"; msgBox.innerText = "Code promo invalide.";
        remisePourcentage = 0; codePromoApplique = "";
    } else {
        const promo = snap.docs[0].data();
        remisePourcentage = promo.pourcentage; codePromoApplique = promo.code;
        msgBox.style.color = "#4CAF50"; msgBox.innerText = `Code appliqué : -${remisePourcentage}% !`;
    }
    mettreAJourPanier();
}
...
function mettreAJourPanier() {
    ...
    if (panier.length === 0) {
        ...
        remisePourcentage = 0; codePromoApplique = ""; document.getElementById('promo-msg').innerText = ''; document.getElementById('input-promo').value = '';
    } else {
    ...
    document.getElementById('zone-total').innerHTML = textTotal;
    window.totalActuel = prixFinal;
}

// ==========================================
// 7. PAIEMENT ET REDIRECTION STRIPE
// ==========================================
...
btnCheckout.addEventListener('click', async () => {
    ...
    try {
        await db.collection('commandes').add({
            email: auth.currentUser.email, articles: panier, total: window.totalActuel, codePromo: codePromoApplique || 'Aucun',
            date: firebase.firestore.FieldValue.serverTimestamp(), statut: 'En cours de préparation'
        });

        const response = await fetch('/.netlify/functions/create-checkout', { 
            method: 'POST', body: JSON.stringify({ panier: panier, remise: remisePourcentage }) 
        });
        ...
        afficherNotification("Erreur serveur."); btnCheckout.innerText = "VALIDER LA COMMANDE"; btnCheckout.disabled = false;
    }
});
